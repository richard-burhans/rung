...
* **`reference_db.py` — the CANNABIS schema.** Every domain table, migration and CRUD helper, and the SQL
  ...
  The `_expr` form exists because `product_observations` carries neither the `potency_implausible` flag nor
  `category_std`, so the append-only history cannot use the column form at all — the same
  cannot-compose-so-it-gets-retyped hazard the natural-flower guard is annotated for below.
...
`docs/store_history_design.md`), and `store_pos_observations`
(append-only INDEPENDENT observations of which menu platform a licensed store runs, sampled from
`dispensaries` rather than from handles held — the instrument that replaces `store_products.platform`
as a market claim; created on its own by `create_store_pos_observations` so an analysis run does not
take the migration suite's ACCESS EXCLUSIVE locks. See)
...
cannabinoids `{NAME: %}` — CBN/CBG/CBC/THCV + acid forms THCA/CBGA/CBDA + delta-8 THC, from
Jane/Hytiva/Cresco/Weedmaps/Dutchie), and `currency`
...
| `capture_mine.py` **[overlay]** | Private overlay module — not shipped in the public core; resolved via the plugin seam (Stage-2/3 catalogs + per-platform helpers; recipe withheld). | — | — |
